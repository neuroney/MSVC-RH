...
SimpleTimingResult runSingleTest(int d, int m, int t, int secpar)
{
    ...
    result.success = true;
    result.maskgen_time = 0.0; // Not used in CH5, but included for consistency
    result.reconstruct_time = 0.0; // Not used in CH5, but included for consistency
...
TestResultData MSVC_CH_5_TIMETEST(int d, int m, int t, int secpar, int iterations, bool silent)
{
    if (!silent)
    {
        std::cout << "=========================================" << std::endl;
